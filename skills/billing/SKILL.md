---
name: billing
description: Full invoicing workflow for a go4.fashion order — create billing order, advance invoice (deposit), and final invoice. Works with whatever invoicing system the brand uses.
disable-model-invocation: true
argument-hint: [customer name or order number]
---
...
Use `$ARGUMENTS` to search. Call `list_orders` with customer or order number filter.
...
If products are missing, ask: **"Some products aren't synced to the invoicing system. Sync them now?"**
...
Ask: **"Create an advance invoice (deposit)? If yes, what percentage or amount?"**
