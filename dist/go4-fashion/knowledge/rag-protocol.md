...
The brand's institutional knowledge lives in `ai_knowledge_base`, not in this skill. You MUST follow this protocol to use it effectively. This is the most important rule in the entire skill.
...
- Calculating composition labels → `material_procurement` for label-specific rules
- Ordering materials → `material_procurement` AND `supplier_terms`
...
> "Zauważyłem [obserwacja]. Czy zapisać to jako regułę na przyszłość?"
...
This is how the brand's AI assistant gets smarter over time. Every captured rule is one less question the manager has to answer in the future. The knowledge base is the brand's institutional memory — treat it that way.
